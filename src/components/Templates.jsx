    ...
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400">Gagnez du temps avec des messages pré-rédigés</p>
        </div>
        <button
      ...
      {/* Add template form */}
      {showAddForm && (
        <div className="glass-dark rounded-2xl p-6 border border-purple-500/20 animate-fade-in">
          <h3 className="text-xl font-bold text-white mb-4">Créer un nouveau template</h3>
          <div className="space-y-4">
            ...
            <div className="flex justify-end space-x-3">
              <button
                ...
                className="px-6 py-2 bg-white/5 hover:bg-white/10 text-gray-300 rounded-xl transition-all"
              >
                Annuler
              </button>
              <button
      ...
      {/* Info box */}
      <div className="glass-dark rounded-2xl p-4 border border-blue-500/20">
        <div className="flex items-start space-x-3">
          <MessageSquare className="w-5 h-5 text-blue-400 flex-shrink-0 mt-0.5" />
          <div className="text-sm text-gray-300">
            <strong className="text-blue-300">Variables disponibles :</strong> [Contact], [Entreprise], [Poste], [Date], [Date Entretien], [Compétences], [Votre Nom], [Domaine]
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {templates.map((template) => (
          <div
            key={template.id}
            className="glass-dark rounded-2xl p-6 border border-white/10 hover:border-purple-500/30 transition-all duration-300"
          >
            {/* Header */}
            ...
            {/* Content */}
            <div className="mb-4">
              <pre className="text-sm text-gray-300 whitespace-pre-wrap font-sans bg-white/5 p-4 rounded-xl border border-white/10 max-h-48 overflow-y-auto">
                {template.content}
              </pre>
            </div>
